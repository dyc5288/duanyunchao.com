...
class cls_memcached
{
    ...
    private static function _get_cache_prefix($prefix_code)
    {
        ...
        if (isset($GLOBALS["U_CONFIG"]['MEMCACHED_PREFIX'][$prefix_code]))
        {
            $prefix = $GLOBALS["U_CONFIG"]['MEMCACHED_PREFIX'][$prefix_code];
        }
        else
        {
    ...
    public static function init()
    {
        try
        {
            if (self::$m === null)
            {
                ...
                // 分配多台服务器
                $servers = array();
                foreach ($GLOBALS['U_CONFIG']['memcached'] as $config)
                {
        ...
        {
            debug_print_backtrace();
            die("Memcached Connect Error" . $e->getMessage());
        }
