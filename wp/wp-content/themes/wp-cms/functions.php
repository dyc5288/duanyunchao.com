...
class newspoonOptions {
	...
	function add() {
		...
		add_theme_page(__('Current Theme Options', 'duanyunchao'), __('Current Theme Options', 'duanyunchao'), 'edit_themes', basename(__FILE__), array('newspoonOptions', 'display'));
	}
...
<div class="wrap">
<h2><?php _e('Current Theme Options', 'duanyunchao'); ?></h2>

<table class="form-table">
<tbody>
<tr valign="top">
<th scope="row"><?php _e('Meta', 'duanyunchao'); ?></th>
<td>
<?php _e('Description:', 'duanyunchao'); ?>
<br/>
<input type="text" name="description" id="description" class="code" style="width:98%;" value="<?php echo($options['description']); ?>">
<br/>
<?php _e('Keywords:', 'duanyunchao'); ?> <small><?php _e('( Separate keywords with commas )', 'duanyunchao'); ?></small>
<br/>
<input type="text" name="keywords" id="keywords" class="code" style="width:98%;" value="<?php echo($options['keywords']); ?>">
	...
	<table class="form-table">
			<tbody>
				<tr valign="top">
					<th scope="row"><?php _e('Search', 'duanyunchao'); ?></th>
					<td>
						<label>
							<input name="google_cse" type="checkbox" value="checkbox" <?php if($options['google_cse']) echo "checked='checked'"; ?> />
							 <?php _e('Using google custom search engine.', 'duanyunchao'); ?>
						</label>
						<br/>
						<?php _e('CX:', 'duanyunchao'); ?>
...
<tr valign="top">
<th scope="row">
<?php _e('Notice', 'duanyunchao'); ?>
<br/>
<small style="font-weight:normal;"><?php _e('HTML enabled', 'duanyunchao'); ?></small>
</th>
<td>
<!-- notice START -->
<label>
<input name="notice" type="checkbox" value="checkbox" <?php if($options['notice']) echo "checked='checked'"; ?> />
<?php _e('This notice bar will display at the top of posts on homepage.', 'duanyunchao'); ?>
</label>
<br />
<label>
...
<?php _e('Whether the column two is displayed in the Home', 'duanyunchao'); ?>
</label>
------<?php _e('Who can see?', 'duanyunchao'); ?>
						<label style="margin-left:10px;">
...
<?php _e('Whether the column three is displayed in the Home', 'duanyunchao'); ?>
</label>
------<?php _e('Who can see?', 'duanyunchao'); ?>
						<label style="margin-left:10px;">
...
<?php _e('Whether the column four is displayed in the Home', 'duanyunchao'); ?>
</label>
------<?php _e('Who can see?', 'duanyunchao'); ?>
						<label style="margin-left:10px;">
...
<?php _e('Whether link coperation is displayed on the home page', 'duanyunchao'); ?>
</label>
<br />
<label>
...
<tbody>
<tr valign="top">
<th scope="row"><?php _e('Feed', 'duanyunchao'); ?></th>
<td>
<label>
<input name="feed" type="checkbox" value="checkbox" <?php if($options['feed']) echo "checked='checked'"; ?> />
<?php _e('Custom feed.', 'duanyunchao'); ?>
</label>
<br/>
<?php _e('URL:', 'duanyunchao'); ?> <input type="text" name="feed_url" id="feed_url" class="code" size="60" value="<?php echo($options['feed_url']); ?>">
</td>
</tr>
</tbody>
</table>
		...
		<table class="form-table">
			<tbody>
				<tr valign="top">
					<th scope="row">
						<?php _e('Web Analytics', 'duanyunchao'); ?>
						<br/>
						<small style="font-weight:normal;"><?php _e('HTML enabled', 'duanyunchao'); ?></small>
					</th>
					<td>
						<label>
							<input name="analytics" type="checkbox" value="checkbox" <?php if($options['analytics']) echo "checked='checked'"; ?> />
							 <?php _e('Add web analytics code to your site. (e.g. Google Analytics, Yahoo! Web Analytics, ...)', 'duanyunchao'); ?>
						</label>
						<label>
...
<p class="submit">
<input class="button-primary" type="submit" name="newspoon_save" value="<?php _e('Save Changes', 'duanyunchao'); ?>" />
</p>
</div>
...
echo '<span><strong>&nbsp;&nbsp;&nbsp;</strong>'.$paged.' / '.$max_page.'</span>'; 
    ...
    if($paged != 1){ 
      ...
      _e('come back home page.', 'duanyunchao');
      echo "</a>";
    } 
    ...
    if($paged != $max_page){ 
      ...
      _e('slip to last page.', 'duanyunchao');
      echo "</a>";
    } 
  } 
}
...
/** l10n */
function theme_init(){
	load_theme_textdomain('duanyunchao', get_template_directory() . '/languages');
}
...
function get_recent_comments() {
    ...
    AND comment_author != 'duanyunchao'
...
<li id="comment-<?php comment_ID(); ?>" class="<?php newspoon_comment_class(); ?>">
 ...
 <dd>
 <h3>
 <span class="num"><?php printf(++$commentcount); ?><?php _e('floor', 'duanyunchao'); ?></span><span><?php newspoon_comment_author(); ?></span>

 <?php
 if((get_option('thread_comments')) && ($args['type'] == 'all' || get_comment_type() == 'comment')) :
 $max_depth = get_option('thread_comments_depth');
 echo comment_reply_link(array(
 'reply_text' => __('Reply','duanyunchao'),
 'login_text' => __('Login to reply.','duanyunchao'),
 'depth' => $depth,
 'max_depth' => $max_depth,
 ...
 qc_comment_edit_link(__('Edit', 'duanyunchao'), '<span class="comment-ajax-edit-link"> ', '</span>');
 }
 edit_comment_link(__('Advanced edit', 'duanyunchao'), '<span class="comment-admin-edit-link"> ', '</span>');
 ?>

 </h3>
 <h4><?php _e('Post:', 'duanyunchao'); ?> <?php comment_date('Y-m-d') ?> <?php comment_time('H:i') ?></h4>
 <div class="comment-text">
 <?php if ($comment->comment_approved == '0') : ?>
 <div class="error-tip"><?php _e('Your comment is awaiting moderation.', 'duanyunchao'); ?></div>
 <?php endif; ?>
